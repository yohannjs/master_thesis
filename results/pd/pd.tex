...
The majority of the \acrshort{ari} scorer for all the \acrshort{tsc} models evaluated at two to nine cluster centers are centered around zero.
...
[X] \textbf{If NOT, why do they differ? Is the model with the highest \acrshort{ari} evaluated at a higher cluster number that 2? Attempt to visualize it, if it is not too difficult.}
    * It may seem strange that the ordered lists of \acrshort{dor}s, and \acrshort{ari}s are so different. 
    * The reason for this is not because \acrshort{dor} inherently values sensitivity higher than specificity, but stems from how the \acrshort{dor} is defined. 
    * Recall that $\mathrm{DOR = ( \acrshort{tp} \times \acrshort{tn} )/ (FP \times \acrshort{fn})}$, since the patient diagnoses dataset is skewed in favour of positives \acrshort{tp} has the potential of being as high as 170 
      while \acrshort{tn} can be as high as 30. 
    * Therefore the \acrshort{dor} will be higher for models with a high sensitivity than for models with an equally high sensitivity.
...
The majority of the \acrshort{ari} scores of PVC models applied to identify patient diagnoses are centered around zero, but as one can see from table \ref{tab:pvc_ind_ari}
...
an accuracy of $0.74$, a sensitivity of $0.70$, a specificity of $0.94$ and a \acrshort{dor} of $35.94$.
...
\begin{comment}
[X] \textbf{Comment on spread of \acrshort{dor}.}
    * From the distribution plot in figure \ref{fig:pvsc_hf_dor_sens_spec_dist} one can see that the majority of the \acrshort{dor} scores are close to zero, but there are couple of models that attain \acrshort{dor}s 
      close to 100, and one model that attains a \acrshort{dor} close to 150.
[X] \textbf{Comment on spread of sensitivity and specificity.}
...
\begin{comment}
[0] \textbf{How big is the difference in performance between the models? Can this be confirmed by the number of \acrshort{tp}/TN attained.}
    * From the top table in \ref{tab:pd_compare} one can see that there is a significant difference in performance between the three models included for comparison.
[0] \textbf{Do any of the models score particularily high on sensitivity or specificity? Can this be confirmed by the number of \acrshort{tp}/TN attained.}
    * The \acrshort{tsc} model \textit{gls/all-views/regular/weighted/2} attains the second highest accuracy, sensitivity and specificity of the three models, but also attains the lowest \acrshort{dor}.
    * The \acrshort{tsc} model can also be said to attain the most balanced scores in terms of sensitivity and specificity. 
    * The PVC model \textit{rls-EF/complete/2} attains the highest specificity, second highest \acrshort{dor}, but lowest sensitivity and accuracy of the three models. 
